/*
 * Module: key_expansion (Pipelined)
 * - Input:  A 128-bit master key.
 * - Output: An array of 11 128-bit round keys (Round 0 to Round 10).
 * - Latency: 11 Clock Cycles
 */
module key_expansion_fsm (
    input  logic         clk,
    input  logic         rst,
    input  logic         go,       // Pulse high to begin
    input  logic [127:0] key_in,
    
    output logic         done,        // Goes high when keys are ready
  ...
  reg [7:0] sbox [0:255];
    ...
    initial begin
        ...
        sbox[8'h5c] = 8'h4a; sbox[8'h5d] = 8'h4c; sbox[8'h5e] = 8'h58; sbox[8'h5f] = 8'hcf;//checked correct
    ...
    // Rotates a 32-bit word left by one byte
    ...
    // --- Sequential Key Expansion Logic ---
    logic [3:0] round_ctr;
    logic       calculating;
    logic [31:0] temp_w, prev_w3, prev_w2, prev_w1, prev_w0;
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (go) begin
                ...
                // 1. Fetch previous key words
                prev_w0 = round_keys[round_ctr-1][127:96];
                prev_w1 = round_keys[round_ctr-1][95:64];
                prev_w2 = round_keys[round_ctr-1][63:32];
                prev_w3 = round_keys[round_ctr-1][31:0];

                // 2. Calculate g-function (Rot -> Sub -> XOR RCON)
                // RCON index is [round_ctr - 1]
                temp_w = f_SubWord(f_RotWord(prev_w3)) ^ RCON[round_ctr - 1];

                // 3. Generate new words (Combinational Chain)
                // w[i] = w[i-4] ^ temp_w
                round_keys[round_ctr][127:96] <= prev_w0 ^ temp_w;                      // New w0
                round_keys[round_ctr][95:64]  <= prev_w1 ^ (prev_w0 ^ temp_w);          // New w1
                round_keys[round_ctr][63:32]  <= prev_w2 ^ (prev_w1 ^ (prev_w0 ^ temp_w)); // New w2
                round_keys[round_ctr][31:0]   <= prev_w3 ^ (prev_w2 ^ (prev_w1 ^ (prev_w0 ^ temp_w))); // New w3
