...
module inv_sbox (
    ...
    // Hard‑coded Inverse S‑box look‑up table (256 entries of 8‑bit values)
    ...
    // InverseSubBytes operation: parallel substitution for all 16 bytes
    always @(*) begin
        for (i = 0; i < 4; i = i + 1) begin
            for (j = 0; j < 4; j = j + 1) begin
                out_state[i][j] = inv_sbox[in_state[i][j]];
            end
        end
    end
